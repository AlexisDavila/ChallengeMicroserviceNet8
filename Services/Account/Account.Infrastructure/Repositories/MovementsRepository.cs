...
public class MovementsRepository : IMovementRepository
{
   ...
   public async Task<List<Movement>> GetMovementsAsync(string accountId)
   {
      await using var connection = new NpgsqlConnection(_configuration.GetValue<string>("DatabaseSettings:ConnectionString"));
      var movements = await connection.QueryFirstOrDefaultAsync<List<Movement>>
   ...
   public Task<bool> UpdateMovementAsync(Movement movement)
   {
      // Implementation for updating a movement
      throw new NotImplementedException();
   }

   public Task<bool> DeleteMovementAsync(Guid movementId)
   {
